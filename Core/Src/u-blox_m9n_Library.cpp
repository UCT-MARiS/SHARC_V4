...
bool SFE_UBLOX_GNSS::begin(UART_HandleTypeDef *huart, uint16_t maxWait, bool assumeSuccess)
{
  ...
  _serialPort = huart; // Grab which port the user wants us to use
  _signsOfLife = false;      // Clear the _signsOfLife flag. It will be set true if valid traffic is seen.
...
void SFE_UBLOX_GNSS::enableDebugging(UART_HandleTypeDef *huart, bool printLimitedDebug)
{
  _debugSerial = huart; // Grab which port the user wants us to use for debugging
...
{
  if (_outputPort != NULL)
	  HAL_UART_Transmit(_outputPort, &incoming, 8, HAL_MAX_DELAY);
...
void SFE_UBLOX_GNSS::processNMEA_v(char incoming)
{
  ...
  if (_nmeaOutputPort != NULL)
	HAL_UART_Transmit(_nmeaOutputPort, (uint8_t *)&incoming, 8, HAL_MAX_DELAY); // Echo this byte to the serial port
}
...
sfe_ublox_status_e SFE_UBLOX_GNSS::sendI2cCommand(ubxPacket *outgoingUBX, uint16_t maxWait)
{
  ...
  while (bytesLeftToSend > 0)
  {
    ...
    {
      uint8_t ubx1 = UBX_SYNCH_1;
      uint8_t ubx2 = UBX_SYNCH_2;
      HAL_I2C_Master_Transmit(_i2cPort, _gpsI2Caddress << 1, &ubx1, 1, HAL_MAX_DELAY);
      HAL_I2C_Master_Transmit(_i2cPort, _gpsI2Caddress << 1, &ubx2, 1, HAL_MAX_DELAY);
      HAL_I2C_Master_Transmit(_i2cPort, _gpsI2Caddress << 1, &(outgoingUBX->cls), 1, HAL_MAX_DELAY);
      HAL_I2C_Master_Transmit(_i2cPort, _gpsI2Caddress << 1, &(outgoingUBX->id), 1, HAL_MAX_DELAY);

      uint8_t LSB = outgoingUBX->len & 0xFF;
	  uint8_t MSB = outgoingUBX->len >> 8;

      HAL_I2C_Master_Transmit(_i2cPort, _gpsI2Caddress << 1, &LSB, 1, HAL_MAX_DELAY);
      HAL_I2C_Master_Transmit(_i2cPort, _gpsI2Caddress << 1, &MSB, 1, HAL_MAX_DELAY);
      ...
      {
        HAL_I2C_Master_Transmit(_i2cPort, _gpsI2Caddress << 1, &(outgoingUBX->payload[startSpot + x]), 1, HAL_MAX_DELAY);
        bytesSent++;
      }
      startSpot += x;
      ...
      {
        // Write checksum
        HAL_I2C_Master_Transmit(_i2cPort, _gpsI2Caddress << 1,&(outgoingUBX->checksumA), 1, HAL_MAX_DELAY);
        HAL_I2C_Master_Transmit(_i2cPort, _gpsI2Caddress << 1,&(outgoingUBX->checksumB), 1, HAL_MAX_DELAY);
        bytesSent += 2;
      }
    }
...
bool SFE_UBLOX_GNSS::pushRawData(uint8_t *dataBytes, size_t numDataBytes, bool stop)
{
  ...
  if (commType == COMM_TYPE_SERIAL)
  {
	...
	size_t bytesWritten = HAL_UART_Transmit(_serialPort, dataBytes, numDataBytes, HAL_MAX_DELAY);
...
void SFE_UBLOX_GNSS::setNMEAOutputPort(UART_HandleTypeDef *nmeaOutputPort)
{
  _nmeaOutputPort = nmeaOutputPort; // Store the port from user
}

void SFE_UBLOX_GNSS::setOutputPort(UART_HandleTypeDef *outputPort)
{
...
bool SFE_UBLOX_GNSS::addGeofence(int32_t latitude, int32_t longitude, uint32_t radius, unsigned char confidence, unsigned char pinPolarity, unsigned char pin, uint16_t maxWait)
{
