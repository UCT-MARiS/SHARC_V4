/*
 * u-blox_m9n_Library.h
 *
 *  Created on: Aug 9, 2024
 */

#ifndef INC_U_BLOX_M9N_LIBRARY_H_
#define INC_U_BLOX_M9N_LIBRARY_H_
...
#define defaultMaxWait 1100
 ...
 // bool begin(I2C_HandleTypeDef *wirePort, uint8_t deviceAddress = 0x42, uint16_t maxWait = defaultMaxWait, bool assumeSuccess = false); // Returns true if module is detected
  // serialPort needs to be perviously initialized to correct baud rate
  bool begin(UART_HandleTypeDef *huart, uint16_t maxWait = defaultMaxWait, bool assumeSuccess = false); // Returns true if module is detected
 ...
 // bool getI2cStopRestart(void) { return (_i2cStopRestart); };
  ...
  void enableDebugging(UART_HandleTypeDef *huart, bool printLimitedDebug = false); // Given a port to print to, enable debug messages. Default to all, not limited.
...
#define defaultMGAdelay 7 // Default to waiting for 7ms between each MGA message
  ...
  #define defaultNavDBDMaxWait 3100
    ...
    bool addGeofence(int32_t latitude, int32_t longitude, uint32_t radius, unsigned char confidence = 0, unsigned char pinPolarity = 0, unsigned char pin = 0, uint16_t maxWait = defaultMaxWait); // Add a new geofence
...
// Depending on the ubx binary response class, store binary responses into different places
  ...
  UART_HandleTypeDef *_serialPort;            // The generic connection to user's chosen Serial hardware
  UART_HandleTypeDef *_nmeaOutputPort = NULL; // The user can assign an output port to print NMEA sentences if they wish
  UART_HandleTypeDef *_debugSerial;           // The stream to send debug messages to if enabled
...
#endif /* INC_U_BLOX_M9N_LIBRARY_H_ */
